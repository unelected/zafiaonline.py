...
class WebClient:
    ...
    def encode_auth_header(self) -> str:
        return base64.b64encode(f"{self.client.id}=:={self.client.token}"
                                .encode("utf-8")).decode("utf-8").strip()

    def logout(self) -> dict:
        response = (requests.post
                    (f"{self.client.rest_address}/user/sign_out",
                     headers=self.auth_headers).json())
        return response

    def sign_up(self, nickname: str, email: str, password,
                language: Languages = Languages.RUSSIAN) -> dict:
        data = {
            "email": email,
            "username": nickname,
        ...
        response = requests.post(f"{self.client.rest_address}/user/sign_up",
                                 data=data,
    ...
    def user_change_sex(self, sex: Sex) -> dict:
        ...
        response = requests.post(f"{self.client.rest_address}/user/change/sex",
    ...
    def user_email_verify(self, language: Languages = Languages.RUSSIAN)\
        ...
        response = requests.post(f""
                                 f"{self.client.rest_address}/user/email/verify",
                                 data=data, headers=self.auth_headers).json()
        return response
    ...
    def user_get(self, _id: str) -> None:
        ...
        response = requests.post(f"{self.client.rest_address}/user/get",
                                 data=data, headers=self.auth_headers).json()
        return response
